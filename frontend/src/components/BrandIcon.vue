...
<template>
  <svg v-if="brand" viewBox="0 0 24 24" role="img" :aria-label="brand.title" class="h-full w-full">
    <path :d="brand.path" :fill="brand.hex" />
  </svg>
  <MingcuteIcon v-else name="server" class="h-full w-full text-slate-700" />
</template>
